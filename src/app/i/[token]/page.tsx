...
export default async function InvitationPage({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  return <main className="invitation-page"><div className="invitation-shell">
    <header className="player-header"><div className="player-brand"><span className="brand-mark">P</span><span><strong>{env.CLUB_NAME}</strong><small>Convocación personal</small></span></div>{answered ? <a className="quiet-button" href={`/api/player/session/${token}`}>Mis partidos</a> : null}</header>
    <section className="invitation-hero">
      ...
      <div className="invitation-essentials">
        <div><span className="essential-icon">◷</span><span><small>Fecha y hora</small><strong>{formatDateTime(invitation.event.startsAt)}</strong></span></div>
        <div><span className="essential-icon">⌖</span><span><small>Lugar</small><strong>{invitation.event.venue}{invitation.event.address ? ` · ${invitation.event.address}` : ""}</strong></span></div>
      </div>
    </section>

    {query.ok ? <section className="response-success"><span className="success-check">✓</span><div><p>Respuesta registrada</p><h2>{playerResponseLabel(invitation.response)}</h2></div><a className="button button-primary" href={`/api/player/session/${token}`}>Ver mis partidos</a></section> : null}
    ...
    <section className="invitation-response card">
      ...
      {closed ? <div className="alert alert-error">La convocatoria está cerrada o ha superado la fecha límite.</div> : <form action={`/api/invitations/${token}/respond`} method="post" className="response-buttons">
        ...
        <details className="response-note"><summary>Añadir una observación</summary><textarea id="note" name="note" maxLength={500} placeholder="Por ejemplo: llego diez minutos tarde" disabled={env.DEMO_MODE} /></details>
      </form>}
      <p className="response-deadline">Puedes responder hasta {formatDateTime(invitation.event.deadlineAt)}.</p>
    </section>
