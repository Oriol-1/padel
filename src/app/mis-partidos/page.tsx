...
export default async function MyMatchesPage({ searchParams }: { searchParams: Promise<{ preview?: string }> }) {
  ...
  if (!portal) return <main className="player-portal"><div className="player-shell player-shell-narrow">
    <div className="player-brand"><span className="brand-mark">P</span><span><strong>{env.CLUB_NAME}</strong><small>Área del jugador</small></span></div>
    <section className="player-empty card"><span className="empty-icon">↗</span><h1>Abre tu enlace personal</h1><p className="muted">Para proteger tus convocatorias, entra desde el enlace que has recibido por WhatsApp.</p><Link className="button button-secondary" href="/">Volver al inicio</Link></section>
  </div></main>;
